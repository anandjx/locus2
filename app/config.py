...
class AgentConfig:
    ...
    def __post_init__(self) -> None:
        if USE_VERTEX_AI:
            ...
            # --- THE SOTA ARCHITECTURAL FIX ---
            # Vertex AI crashes if you pass a staging bucket with location="global".
            # Since Locus is deployed via Cloud Run (not Agent Engine), the bucket 
            # is not required for runtime. We safely omit it to enable global routing.
            if self.GOOGLE_CLOUD_LOCATION.lower() == "global":
                vertexai.init(
                    project=self.GOOGLE_CLOUD_PROJECT,
                    location=self.GOOGLE_CLOUD_LOCATION,
                    # Staging bucket intentionally omitted to prevent startup crash
                )
            else:
                if not self.GOOGLE_CLOUD_STAGING_BUCKET:
                    raise RuntimeError(
                        "GOOGLE_CLOUD_STAGING_BUCKET is required for regional deployment"
                    )
                vertexai.init(
                    project=self.GOOGLE_CLOUD_PROJECT,
                    location=self.GOOGLE_CLOUD_LOCATION,
